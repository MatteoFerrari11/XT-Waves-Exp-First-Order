...
%
% This code generates Figure 1 with C1 splines 
...
for p = p1:p2
    ...
    for ii = N1:N2
        ...
        %special for this example to compute newton potential
        ...
        dtU_app = zeros(size(dtU_ex));
        V_app = zeros(size(dtU_ex));

        dxU_ex = dxU(t_plot',x_plot);
        dxU_app = zeros(size(dtU_ex));
        
        dtV_N_app = zeros(size(dtU_ex));
        ...
        err_V = sum(sum(abs(V_app-dtU_ex).^2))/N_plot^2;
        ...
        err_norm_call_V(cont_p,cont_x) = sqrt((err_V+err_dtVN+err_dtU+err_dxU)./(rel_V+rel_dtVN+rel_dtU+rel_dxU))

    end

end

plot_error({}, err_norm_call_V(1,:),(T./2.^(N1+1))./(T./2.^(N1+1:N2+1)),1);
plot_error({}, err_norm_call_V(2,:),(T./2.^(N1+1))./(T./2.^(N1+1:N2+1)),1);
plot_error({}, err_norm_call_V(3,:),(T./2.^(N1+1))./(T./2.^(N1+1:N2+1)),1);
plot_error({'p=2','p=3','p=4','p=5'}, err_norm_call_V(4,:),(T./2.^(N1+1))./(T./2.^(N1+1:N2+1)),1);
legend({'$p=2$','$p=3$','$p=4$','$p=5$'}, 'Interpreter', 'latex','FontSize', 16.7);
set(gca,'FontSize', 14);
