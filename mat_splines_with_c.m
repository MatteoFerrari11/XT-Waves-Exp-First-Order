function K = aa_mat_splines_with_c(Nt,p,r,k1,k2,knots,T1,T2,c)
...
% i.e. we have
% K[i,j] = (\partial_t^{k_1} \phi_j^p, c \partial_t^{k_2} \phi_i^p)_{L^2(0,T)}
...
% K[i,j] = (c dt^{k_1} \phi_j^p, c dt^{k_2} \phi_i^p)_{L^2(T1,T2)}
...
for jj = 1 : siz
    for ii = 1 : siz
        ...
        for k = max([1,floor(jj/(p-r))-(p+r)]):min([jj,Nt])
            ...
            K(jj,ii) = K(jj,ii) + sum(phi_2.*phi_1.*ws.*c(xs));
        end
    end
end
