...
%
% This code generates Figures 2,3,4 with C1 splines
...
for p = p1:p2
    ...
    %error estimates
    if N1 < N2 && Nx == Nt
        plot_error({}, err_norm_call_V(cont_p,:),T./2.^(N1+1:N2+1),1);
        plot_error({'error norm energy','L2 error U'}, err_U(cont_p,:), T./2.^(N1+1:N2+1),1);
        title(['Various errors in U for splines with px=' num2str(p) 'and pt=' num2str(p)],'FontSize',16,'Interpreter','latex')
        xlabel('h')
    end
end
...
plot_error({'p=2','p=3','p=4','p=5'}, err_norm_call_V(4,:),T./2.^(N1+1:N2+1),1);
...
plot_error({'p=2','p=3','p=4','p=5'}, err_U(4,:),T./2.^(N1+1:N2+1),2);
legend({'$p=2$','$p=3$','$p=4$','$p=5$'}, 'Interpreter', 'latex','FontSize', 16.7);
set(gca, 'XTick', [0.05, 0.07, 0.09,0.11]); 
set(gca,'FontSize', 14);
xlabel('$h_t=h_x$', 'Interpreter', 'latex','FontSize', 18);
...
plot_error({'p=2','p=3','p=4','p=5'}, err_V(4,:),T./2.^(N1+1:N2+1),3);
legend({'$p=2$','$p=3$','$p=4$','$p=5$'}, 'Interpreter', 'latex','FontSize', 16.7);
set(gca, 'XTick', [0.05, 0.07, 0.09,0.11]); 
set(gca,'FontSize', 14);
xlabel('$h_t=h_x$', 'Interpreter', 'latex','FontSize', 18);